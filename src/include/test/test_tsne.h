...
void test_bhtsne(int N, int NDIMS) {
    ...
    printf("Starting TSNE calculation with %u points.\n", N);
    cudaEventRecord(start);
    BHTSNE::tsne(dense_handle, sparse_handle, thrust::raw_pointer_cast(h_X.data()), N, NDIMS, 2, 45.0, 500.0, 12.0, 1000, 1000, 0.0);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    float milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop);
...
void test_bhtsne_full_mnist(std::string fname) {
    ...
    printf("Starting TSNE calculation with %u points.\n", num_images);
    cudaEventRecord(start);
    BHTSNE::tsne(dense_handle, sparse_handle, data, num_images, num_columns*num_rows, 2, 45.0, 500.0, 12.0, 1000, 1000, 0.0);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    float milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop);
...
void test_bhtsne_full_cifar100(std::string fname) {
    ...
    printf("Starting TSNE calculation with %u points.\n", num_images);
    cudaEventRecord(start);
    BHTSNE::tsne(dense_handle, sparse_handle, data, num_images, num_channels*num_columns*num_rows, 2, 45.0, 500.0, 12.0, 1000, 1000, 0.0);
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    float milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop);
