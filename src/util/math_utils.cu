...
thrust::device_vector<float> square(const thrust::device_vector<float> &vec, const unsigned int N) {
    ...
    thrust::transform(vec.begin(), vec.end(), squared_vals.begin(), tfunc::square());
    return squared_vals;
}
...
thrust::device_vector<float> sqrt(const thrust::device_vector<float> &vec, const unsigned int N) {
    ...
    thrust::transform(vec.begin(), vec.end(), sqrt_vals.begin(), tfunc::sqrt());
    return sqrt_vals;
}

void max_norm(thrust::device_vector<float> &vec) {
    float max_val = thrust::transform_reduce(vec.begin(), vec.end(), tfunc::abs(), 0.0f, thrust::maximum<float>());
